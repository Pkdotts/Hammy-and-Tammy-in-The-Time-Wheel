...
func _ready():
	...
	if menu_parent:
		menu_parent.connect("item_rect_changed", Callable(self, "refresh_pos").bind(false))
	if highlight:
		if visible:
			turn_on_highlight()
		connect("visibility_changed", turn_on_highlight)
		connect("visibility_changed", unhighlight)
...
func set_cursor_from_index(index : int, transition = true, play_sfx = false) -> void:
	...
	var new_position = menu_item.global_position + cursor_offset + size
#	var size = menu_item.rect_size
	...
	if transition:
		...
		if sprite_frames == null:
			...
			tween.tween_property(self, "scale", Vector2(0.8, 1.2), 0.03).set_delay(0.1)
			tween.tween_property(self, "scale", Vector2(1, 1), 0.03).set_delay(0.13)
		scale = Vector2.ONE
...
func _connect_to_menu_parent(new_parent, old_parent = null):
	if old_parent != null:
		old_parent.disconnect("item_rect_changed", refresh_pos)
	new_parent.connect("item_rect_changed", Callable(self, "refresh_pos").bind(false))
